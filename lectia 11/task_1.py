...
def task_5(*args, **kwargs):
    ...
    lista_stringuri = sorted([key for key, value in kwargs.items() if isinstance(value, str)])
...
def task_9(*args, number):
    multiples = [arg for arg in args if arg % number == 0]
    return multiples

result = task_9(1, 2, 3, 4, 5, number=2)
print(result)  
...
def task_10(*args, number):
    ...
    for arg in args:
        if arg % number == 0:  
            divisible_args.append(arg)
    return divisible_args
divizibile = task_10(1, 2, 3, 4, 5, 6, 7, 8, number=6)  
print(divizibile)  
...
def are_anagrams(*args):
    first_word = sorted(args[0]) 
    for word in args[1:]:
        if sorted(word) != first_word:
            return False
    return True

def task_13(base_word, *args):
    return are_anagrams(base_word, *args)
...
def task_14(*args, sub_string):   
    ...
    for arg in args:    
        if sub_string in arg:
           containing_substring.append(arg)
    return containing_substring
...
def task_17(number, **kwargs):
    ...
    for operation, nums in kwargs.items():
        if operation == 'add':
            result += sum(nums)
        elif operation == 'sub':
            result -= sum(nums)
...
# CODUL TĂU VINE MAI JOS:
def is_prime(n):
    #Verificam daca un număr este prim
    if n <= 1:
        return False
    if n <= 3:
        return True
...
print(task_19(1, 2, 3, 4, 5, 6, 7, 8, 9))  
# Output: {2: 1, 3: 1, 5: 1, 7: 1}
